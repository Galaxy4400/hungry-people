...
function popup_open(item, video = '') {
	...
	if (curent_popup && unlock) {
		...
		if (!document.querySelector('.mobile-menu._active')) {
			body_lock_add(500);
		}
...
function popup_close(item, bodyUnlock = true) {
	if (unlock) {
		...
		if (!document.querySelector('.mobile-menu._active') && bodyUnlock) {
			body_lock_remove(500);
		}
...
function select_item(select) {
	...
	select_parent.insertAdjacentHTML('beforeend',
		...
		'<div class="select__options" data-simplebar>' + select_get_options(select_options) + '</div>' +
		'</div></div>');
			...
			if (input.classList.contains('_date')) {
				const calendarItem = datepicker(input, {
					...
					formatter: (input, date, instance) => {
						const mm = date.toLocaleDateString('ru', { month: '2-digit' });
						const dd = date.toLocaleDateString('ru', { day: '2-digit' });
						input.value = mm + '.' + dd;
					},
...
let slider = new Swiper('.special-slider__body', {
	autoplay: {
		delay: 8000,
...
function init() {
	...
	var myMap = new ymaps.Map("map", {
		controls: [],
		center: [55.75481081, 37.62142781],
		zoom: 15
	});
